<template>
  <main>
    <Drawer v-model:visible="drawerOpen" header="Menu">
      <template #footer>
        ...
        <div class="flex items-center justify-between">
          ...
          <Button label="Cerrar sesión" icon="pi pi-sign-out" class="!text-xs" severity="danger" text @click="logout"
            size="small" />
        </div>
      </template>
    ...
    <header
      ...
      }" icon="pi pi-bars" text severity="secondary" size="small" @click="drawerOpen = !drawerOpen"
        class="lg:!hidden" />
      <h2 class="text-xl font-normal flex items-center">
        <Icon name="lucide:church" class="mr-2 hidden md:block" />
        Cementerio - &nbsp;<span class="font-semibold"> Auditor</span>
      </h2>
      <div class="items-center gap-x-4 hidden lg:flex">
        ...
        }" icon="pi pi-sign-out" severity="danger" text @click="logout" size="small" />
      </div>
    </header>
...
const sidebarItems = [
  {
    label: "Navegación",
    items: [
      { label: "Inicio", icon: "lucide:house", to: "/auditor" },
      { label: "Perfil", icon: "lucide:user", to: "/auditor/profile" },
